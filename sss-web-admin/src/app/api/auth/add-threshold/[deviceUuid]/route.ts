...
/**
 * POST handler for adding a new threshold for a device
 * @param {NextRequest} request - The incoming request
 * @param {Object} context - Route context
 * @param {Promise<{ deviceUuid: string }>} context.params - Route parameters
 * @returns {Promise<NextResponse>} Response with add threshold result or error
 */
export async function POST(
  ...
  try {
    ...
    console.log('Checking authentication for add-threshold route...');
    ...
    console.log('Authentication successful, proceeding to add threshold...');
    ...
    // Validate that threshold is an object
    ...
    const externalApiUrl = `${baseUrl}/api/v1/threshold/add_alert/${deviceUuid}`;
    ...
    if (axios.isAxiosError(error)) {
      ...
      return NextResponse.json(
        { error: `Failed to add threshold: ${message}` },
        { status }
      );
    }
    
    return NextResponse.json(
      { error: 'Failed to add threshold', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
