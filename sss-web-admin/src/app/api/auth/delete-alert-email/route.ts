...
/**
 * Interface for delete email request body
...
/**
 * Interface for delete email response
...
/**
 * DELETE handler for deleting an alert email
 * @param {NextRequest} request - The incoming request
 * @returns {Promise<NextResponse>} Response with deletion result or error
 */
export async function DELETE(request: NextRequest) {
  try {
    // Check authentication
    console.log('Checking authentication for delete-alert-email route...');
    ...
    console.log('Authentication successful, proceeding to delete email...');

    // Parse request body
    const body: DeleteEmailRequest = await request.json();
    const { email } = body;
    ...
    const externalApiUrl = `${baseUrl}/api/v1/threshold/delete_email_alert`;
    ...
    const response = await axios.delete<DeleteEmailResponse>(
      externalApiUrl,
      {
    ...
    return NextResponse.json(
      { error: 'Failed to delete alert email', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
