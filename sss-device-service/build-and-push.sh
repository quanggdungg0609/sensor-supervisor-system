...
IMAGE_NAME="${IMAGE_NAME:-sss-device-service}"
...
show_help() {
    ...
    echo "Options:"
    echo "  --registry URL          Registry URL (REQUIRED for push)"
    echo "  --username USER         Registry username (REQUIRED for push)"
    echo "  --password PASS         Registry password (REQUIRED for push)"
    echo "  --group GROUP           Image group/namespace (default: quangdung)"
    echo "  --name NAME             Image name (default: sss-device-service)"
    echo "  --no-push               Build image but don't push to registry"
    echo "  --no-compose-update     Don't update docker-compose.yaml"
    echo "  --help                  Show this help message"
    echo ""
    echo "Environment Variables (REQUIRED for push):"
...
validate_credentials() {
    ...
    if [ ${#missing_params[@]} -gt 0 ]; then
        print_error "Missing required parameters for push:"
        for param in "${missing_params[@]}"; do
            echo "  - $param"
        done
        echo ""
        print_error "Please provide registry credentials or use --no-push"
        exit 1
    fi
}
...
build_image() {
    ...
    print_success "JVM Docker image built: $image_tag"
...
show_image_info() {
    ...
    echo "Build Type: JVM Image"
    
    if [ "$push_image" = true ]; then
        echo "Registry Image: ${REGISTRY_URL}/${IMAGE_GROUP}/${IMAGE_NAME}:${version}"
        echo "Registry Latest: ${REGISTRY_URL}/${IMAGE_GROUP}/${IMAGE_NAME}:latest"
    else
    ...
    echo
    print_step "Image size information:"
    docker images "${IMAGE_GROUP}/${IMAGE_NAME}" --format "table {{.Repository}}:{{.Tag}}\t{{.Size}}\t{{.CreatedAt}}"
}

# Main execution
main() {
    echo "=== SSS Device Service - JVM Docker Build and Push Script ==="
    echo
    ...
    show_image_info $VERSION $PUSH_IMAGE
}
